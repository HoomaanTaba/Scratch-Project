...
void renderWorkspace(SDL_Renderer *renderer, TTF_Font *font) {
    for (auto &wb: workspaceBlocks) {
        ...
        if (wb.hasNumberInput || wb.hasTextInput) {
            ...
            if (wb.hasTextInput)
                ...
                text = wb.inputText1.empty()
                           ? to_string((int) wb.inputValue)
                              : wb.inputText1;
            wb.inputRect1 = {wb.rect.x + wb.rect.w - 60, wb.rect.y + 12, 50, 20};
            ...
            text = wb.inputText1.empty()
                              ? to_string((int) wb.inputValue)
                              : wb.inputText1;
            drawTextCentered(renderer, font, text, wb.inputRect1, textColor);
        }
        ...
        if (wb.hasSecondNumberInput) {
            ...
            SDL_Color textColor = {0, 0, 0, 255};
            string text = wb.inputText2.empty() ? to_string((int) wb.inputValue2) : wb.inputText2;
            drawTextCentered(renderer, font, text, wb.inputRect2, textColor);
        }
        ...
        if (wb.hasDropdown) {
            ...
            SDL_Color textColor = {0, 0, 0, 255};
            drawTextCentered(renderer, font, wb.dropdownOptions[wb.selectedOption], wb.dropdownRect, textColor);
            ...
            if (wb.dropdownOpen) {
                for (int i = 0; i < wb.dropdownOptions.size(); i++) {
                    ...
                    drawTextCentered(renderer, font, wb.dropdownOptions[i], optionRect, textColor);
                }
            }
        }
        // Reporter output
        if (!wb.reporterOutput.empty()) {
            SDL_Color color = {20, 20, 20, 255};
            SDL_Surface *surf = TTF_RenderUTF8_Blended(font, wb.reporterOutput.c_str(), color);
            ...
            if (surf) {
                ...
                outRect.w = surf->w;
                outRect.h = surf->h;
                outRect.x = wb.rect.x + 10;
                outRect.y = wb.rect.y + wb.rect.h + 4;

                SDL_RenderCopy(renderer, text, NULL, &outRect);
...
void executeBlockChain(int startID, Sprite &sprite, Stage &stage) {
    ...
    block->reporterOutput.clear();

    if (block->hasNumberInput && !block->inputText1.empty())
        block->inputValue = atof(block->inputText1.c_str());

    if (block->hasSecondNumberInput && !block->inputText2.empty())
        block->inputValue2 = atof(block->inputText2.c_str());
    ...
    switch (block->blockType) {
        ...
        case BLOCK_THINK_FOR: {
            think_for_seconds(sprite, block->inputText1, block->inputValue2);
            break;
        }
        ...
        case BLOCK_CHANGE_EFFECT: {
            if (block->selectedOption >= 0 &&
                block->selectedOption < block->dropdownOptions.size())
                change_effect(sprite,
                              block->dropdownOptions[block->selectedOption],
                              block->inputValue);
            break;
        }
        ...
        case BLOCK_TOUCHING: {
            ...
            block->reporterOutput = result ? "True" : "False";
        }
        break;
        ...
        case BLOCK_DISTANCE: {
            ...
            block->reporterOutput = std::to_string(d);
        }
        break;
        ...
        case BLOCK_KEY_PRESSED: {
            ...
            block->reporterOutput = pressed ? "True" : "False";
        }
        break;
        case BLOCK_MOUSE_DOWN:
            block->reporterOutput = mouseDown() ? "True" : "False";
            break;
        case BLOCK_TIMER:
            block->reporterOutput = std::to_string(timerValue());
            break;
        case BLOCK_RESET_TIMER:
            resetTimer();
            break;
        case BLOCK_TOUCHING_COLOR:
            block->reporterOutput =
                    touchingColor(sprite, {255, 0, 0, 255}) ? "True" : "False";
            break;
        ...
        break;
        case BLOCK_ANSWER:
            block->reporterOutput = getAnswer();
            break;
        case BLOCK_MOUSE_X:
            block->reporterOutput = std::to_string(mouseX());
            break;
        case BLOCK_MOUSE_Y:
            block->reporterOutput = std::to_string(mouseY());
            break;
...
int main(int argc, char *argv[]) {
    ...
    Sprite cat{};
    cat.visible = true;
    cat.texture = loadTexture(m_renderer, "costume1.png");
    ...
    looksBtn.active = false;
    soundBtn.active = false;
    eventsBtn.active = false;
    controlBtn.active = false;
    sensingBtn.active = false;
    ...
    while (running) {
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                ...
                if (isInside(mx, my, CodeTab.rect)) {
                    ...
                    eventsBtn.active =
                            looksBtn.active =
                            soundBtn.active =
                            controlBtn.active = sensingBtn.active = operatorsBtn.active = variablesBtn.active = false;
                    ...
                    eventsBtn.active =
                            looksBtn.active =
                            soundBtn.active =
                            motionBtn.active = sensingBtn.active = operatorsBtn.active = variablesBtn.active = false;
                    ...
                    eventsBtn.active =
                            looksBtn.active =
                            soundBtn.active =
                            controlBtn.active = motionBtn.active = operatorsBtn.active = variablesBtn.active = false;
                    ...
                    eventsBtn.active =
                            looksBtn.active =
                            soundBtn.active =
                            controlBtn.active = sensingBtn.active = motionBtn.active = variablesBtn.active = false;
                    ...
                    eventsBtn.active =
                            looksBtn.active =
                            soundBtn.active =
                            controlBtn.active = sensingBtn.active = operatorsBtn.active = motionBtn.active = false;
                }

                // ======= Execute block on click =====
                SDL_Point p = {mx, my};

                for (auto &b: workspaceBlocks) {
                    // if klickedd on input 1
                    if ((b.hasNumberInput || b.hasTextInput) && SDL_PointInRect(&p, &b.inputRect1)) {
                    ...
                    //if klicked on input 2
                    if (b.hasSecondNumberInput && SDL_PointInRect(&p, &b.inputRect2)) {
                        b.editingInput2 = true;
                        SDL_StartTextInput();
                        break;
                    }

                    //block klicked, so it's active
            ...
            if (CodeTab.active) {
                if (eventsBtn.active)
                    handleEventBlock(e, true, true);

                if (soundBtn.active)
                    handleSoundBlock(e, true, true);

                if (looksBtn.active)
                    handleLooksBlock(e, true, true);

                if (sensingBtn.active)
                    handleSensingBlock(e, true, true);

                if (motionBtn.active)
                    handleMotionBlock(e, true, true);
            }
        }
        // =========================== EXECUTION ENGINE ====================
        ...
        if (CodeTab.active) {
            ...
            // ====== Render Active Menu Blocks ========

            if (motionBtn.active) {
                for (auto &b: MotionMenuBlocks)
                    SDL_RenderCopy(m_renderer, b.texture, NULL, &b.rect);
            }

            if (looksBtn.active) {
                for (auto &b: LooksMenuBlocks)
                    SDL_RenderCopy(m_renderer, b.texture, NULL, &b.rect);
            }

            if (soundBtn.active) {
                for (auto &b: SoundMenuBlocks)
                    SDL_RenderCopy(m_renderer, b.texture, NULL, &b.rect);
            }

            if (eventsBtn.active) {
                for (auto &b: EventMenuBlocks)
                    SDL_RenderCopy(m_renderer, b.texture, NULL, &b.rect);
            }

            if (sensingBtn.active) {
                for (auto &b: SensingMenuBlocks)
                    SDL_RenderCopy(m_renderer, b.texture, NULL, &b.rect);
            }

            renderWorkspace(m_renderer, font);
        }
