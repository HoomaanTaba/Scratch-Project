...
void initEvents(SDL_Renderer* renderer) {
    ...
    menuBlocks.push_back(keyBlock);

    // when sprite clicked
    menuBlocks.push_back({
        spriteClickTex,
        {80, startY+gap*2, 180, 60},
        false, 0, 0,
        true,
        -1,
        globalBlockID++
    ...
    menuBlocks.push_back(backdropBlock);

    // when loudness
    menuBlocks.push_back({
        loudnessTex,
        {80, startY+gap*4, 180, 60},
        false, 0,0,
        true,
        -1,
        globalBlockID++
    ...
    menuBlocks.push_back(receiveBlock);

    // broadcast
    DraggableBlock broadcastBlock = {
            broadcastTex,
            {80, startY+gap*6, 180, 50},
            false, 0, 0,
            false,
            -1,
            globalBlockID++
    ...
    menuBlocks.push_back(broadcastBlock);

    // broadcast and wait
    DraggableBlock broadcastWaitBlock = {
            broadcastWaitTex,
            {80, startY+gap*7, 180, 50},
            false, 0,0,
            false,
            -1,
            globalBlockID++
    ...
    menuBlocks.push_back(broadcastWaitBlock);
}
...
void renderEventBlocks(SDL_Renderer* renderer,TTF_Font * font, bool codeTabActive, bool eventsBtnActive) {
    if(codeTabActive && eventsBtnActive) {
        for(auto& mb : menuBlocks) {
